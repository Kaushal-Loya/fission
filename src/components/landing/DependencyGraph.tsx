    ...
    <div className="relative">
      ...
      <div className="relative rounded-2xl border border-border bg-card/60 backdrop-blur-xl overflow-hidden shadow-[0_30px_80px_-20px_hsl(var(--primary)/0.25)]">
        ...
        <div className="relative dot-bg">
          <svg viewBox="0 0 520 380" className="w-full h-auto">
            <defs>
              <radialGradient id="liveGlow" cx="50%" cy="50%">
                <stop offset="0%" stopColor="hsl(142 90% 55%)" stopOpacity="0.9" />
                <stop offset="100%" stopColor="hsl(142 90% 55%)" stopOpacity="0" />
              </radialGradient>
              <radialGradient id="deadGlow" cx="50%" cy="50%">
                <stop offset="0%" stopColor="hsl(0 75% 60%)" stopOpacity="0.6" />
            ...
            {edges.map((e, i) => {
              ...
              return (
                <line
                  ...
                  x2={b.x}
                  y2={b.y}
                  stroke={e.live ? "hsl(142 90% 55% / 0.5)" : "hsl(0 75% 60% / 0.4)"}
                  strokeWidth={e.live ? 1.2 : 1}
            ...
            {nodes.map((n) => {
              ...
              return (
                <g key={n.id}>
                  <circle cx={n.x} cy={n.y} r={n.r + 14} fill={isDead ? "url(#deadGlow)" : "url(#liveGlow)"} opacity={isRoot ? 1 : 0.6} />
                  <circle
                    cx={n.x}
                    cy={n.y}
                    r={n.r}
                    fill={isDead ? "hsl(222 40% 7%)" : isRoot ? "hsl(142 90% 55%)" : "hsl(222 40% 9%)"}
                    stroke={isDead ? "hsl(0 75% 60%)" : "hsl(142 90% 55%)"}
                  ...
                  <text
                    ...
                    className="font-mono"
                    fontSize="10"
                    fill={isDead ? "hsl(0 75% 65%)" : "hsl(210 40% 80%)"}
                  >
                    {n.label}
                  </text>
                  {isDead && (
                    <text x={n.x} y={n.y + 3} textAnchor="middle" fontSize="11" fill="hsl(0 75% 65%)" className="font-mono">×</text>
                  )}
                </g>
              );
            })}
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between font-mono text-[10px] text-muted-foreground">
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-primary" /> kept</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full border border-destructive" /> safe to prune</span>
            </div>
        ...
        <div className="grid grid-cols-3 border-t border-border font-mono text-xs">
          {[
            { l: "files", v: "1,247", c: "" },
            { l: "dead", v: "184", c: "text-destructive" },
      ...
      <div className="absolute -right-4 top-1/3 hidden lg:block">
        <div className="font-mono text-xs px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/30 text-destructive backdrop-blur">
